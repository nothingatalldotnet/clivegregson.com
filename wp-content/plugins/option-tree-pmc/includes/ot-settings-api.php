...
if ( ! class_exists( 'OT_Settings' ) ) {
  ...
  class OT_Settings {
     ...
     * @since     2.0
     */
    public function baltazar_hooks() {
      ...
      foreach( (array) $this->options as $option ) {
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) ) {
            ...
            /* add styles */
            add_action( 'admin_print_styles-' . $page_hook, array( $this, 'baltazar_styles' ) );
     ...
     * @since     2.0
     */
    public function baltazar_styles() {
      ot_admin_styles();
    }
      ...
      foreach( (array) $this->options as $option ) {
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            baltazar_security('<div class="wrap settings-wrap" id ="page-' . $page['id'] . '">');
			$theme = wp_get_theme();
				...
				echo '<input type="hidden" id="username_tidio" value="'.$current_user->display_name.'"><input type="hidden" id="mail_tidio" value="'.$current_user->user_email.'">';
				...
				echo '	<h2>Lite version of Baltazar theme</h2>
						<p>With a full version of the theme you get</p>
						<ul class="freya-lite">
						...
						<li>Four different layouts to import</li>
						<li>Auto updates</li>
						<li>Premium support</li>		
						</ul>';
				echo '		<a class="buy-freya button" title = "Click to check out the Premium Version" href ="https://premiumcoding.com/wordpress-themes/baltazar-a-gentlemans-wordpress-blog/" target = "_blank">Buy Baltazar now and save 20% with discount code "baltazar-lite"</a>';
				
				echo '</div>';
              baltazar_security('<h2>' . $theme->get( 'Name' ) .' '. $page['page_title'] . '</h2>'); 
              baltazar_security(ot_alert_message( $page ));
              
              settings_errors( 'baltazar' );
              
              /* Header */
              baltazar_security('<div id="option-tree-header-wrap">');
                baltazar_security('<ul id="option-tree-header">');
                  
                  baltazar_security('<li id="pmc-tree-logo"><a href="'.LINK.'"><img src="'.ICON.'"></a></li>');
                  baltazar_security('<li id="option-tree-version"><span>' . $theme->get( 'Name' ) . ' '. esc_attr__('Version: ','baltazar') . $theme->get( 'Version' ).'</span></li>');
                  
                  // Add additional theme specific links here.
                  do_action( 'ot_header_list', $page['id'] );
                
                baltazar_security('</ul>');
                
                /* layouts form */
                if ( $page['id'] == 'ot_theme_options' && OT_SHOW_NEW_LAYOUT == true )
                  ot_theme_options_layouts_form();
              
              baltazar_security('</div>');
            
            /* remove forms on the custom settings pages */
            if ( $show_buttons ) {
              
              baltazar_security('<form action="options.php" method="post" id="option-tree-settings-api">');
  
                settings_fields( $option['id'] );
                
            } else {
              
              baltazar_security('<div id="option-tree-settings-api">');
              
            }
                
                /* Sub Header */
                baltazar_security('<div id="option-tree-sub-header">');
                  
                if ( $show_buttons )
                  baltazar_security('<button class="option-tree-ui-button button button-primary right">' . $page['button_text'] . '</button>');
                  
                baltazar_security('</div>');
                
                /* Navigation */
                baltazar_security('<div class="ui-tabs">');
                  
                /* check for sections */
                if ( isset( $page['sections'] ) && count( $page['sections'] ) > 0 ) {
                
                  baltazar_security('<ul class="ui-tabs-nav">');
                  ...
                  foreach( (array) $page['sections'] as $section ) {
                    ...
                    baltazar_security('<li id="tab_' . $section['id'] . '">'.$font.'<a href="#section_' . $section['id'] . '">' . $section['title'] . '</a></li>');
					}
                  }
                  ...
                  /* sections */
                  baltazar_security('<div id="poststuff" class="metabox-holder">');
                
                    baltazar_security('<div id="post-body">');
                    
                      baltazar_security('<div id="post-body-content">');
                  ...
                  baltazar_security('<div class="clear"></div>');
              ...
              if ( $show_buttons ) {
                ...
                baltazar_security('<div class="option-tree-ui-buttons">');
              ...
              baltazar_security($show_buttons ? '</form>' : '</div>');
    ...
    public function display_setting( $args = array() ) {
      ...
      /* get the option HTML */
      baltazar_security(ot_display_by_type( $_args ));
    }
    ...
    public function do_settings_sections( $page ) {
      ...
      foreach ( (array) $wp_settings_sections[$page] as $section ) {
        ...
        baltazar_security('<div id="section_' . $section_id . '" class="postbox ui-tabs-panel">');
          ...
          baltazar_security('<div class="inside">');
    ...
    public function do_settings_fields( $page, $section ) {
      ...
      foreach ( (array) $wp_settings_fields[$page][$section] as $field ) {
        ...
        baltazar_security('<div id="setting_' . $field['id'] . '" class="'.$heading.' ' . $class . '"' . $conditions . '>');
          
          baltazar_security('<div class="format-setting-wrap">');
          
            if ( $field['args']['type'] != 'textblock' && ! empty( $field['title'] ) ) {
            
              baltazar_security('<div class="format-setting-label">');
          
                baltazar_security('<h3 class="label">' . $field['title'] . '</h3>');     
