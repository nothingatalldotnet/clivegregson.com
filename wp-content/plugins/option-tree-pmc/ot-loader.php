 ...
 */
define('ICON','https://baltazar.premiumcoding.com/wp-content/uploads/2019/01/baltazar-favicon.png');
define('LINK','https://premiumcoding.com');
define('CRYPTO',false);
add_filter( 'ot_theme_mode', '__return_false' );
add_filter( 'ot_theme_plugin', '__return_true' );

require( plugin_dir_path( __FILE__ ) . 'updater/theme-updater.php' );	
...
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' ) && OT_PLUGIN_MODE == true ) {
  ...
  function ot_conflict_notice() {
    ...
    echo '<div class="error"><p>' . esc_html__( 'OptionTree is installed as a plugin and also embedded in your current theme. Please deactivate the plugin to load the theme dependent version of OptionTree, and remove this warning.', 'baltazar' ) . '</p></div>';
  ...
  class OT_Loader {
    ...
    public function load_textdomain() {
      ...
      if ( OT_PLUGIN_MODE ) {
        ...
        load_plugin_textdomain( 'baltazar', false, OT_LANG_DIR );
        
      } else {
      
        load_theme_textdomain( 'baltazar', OT_LANG_DIR );
    ...
    public function load_option_tree() {
      ...
      /* setup the constants */
      $this->baltazar_constants();
      
      /* include the required admin files */
      $this->admin_includes();
      
      /* include the required files */
      $this->baltazar_includes();
     ...
     * @since     2.0
     */
    private function baltazar_constants() {
     ...
     * @since     2.0
     */
    private function baltazar_includes() {
     ...
     * @since     2.0
     */
    private function baltazar_hooks() {
      ...
      add_filter( 'gettext', array( $this, 'change_image_button' ), 10, 3 );
       ...
       /* font Awsome Icons */
      add_action( 'wp_enqueue_scripts', 'ot_load_icons' );	     
    }
    ...
    public function add_list_item() {
      ...
      ot_list_item_view( esc_attr($_REQUEST['name']), esc_attr($_REQUEST['count']), array(), esc_attr($_REQUEST['post_id']), esc_attr($_REQUEST['get_option']), unserialize( ot_decode( esc_attr($_REQUEST['settings']) ) ), esc_attr($_REQUEST['type']) );
      die();
    }
    ...
    public function add_social_links() {
      check_ajax_referer( 'option_tree', 'nonce' );
      ot_social_links_view( esc_attr($_REQUEST['name']), esc_attr($_REQUEST['count']), array(), esc_attr($_REQUEST['post_id']), esc_attr($_REQUEST['get_option']), unserialize( ot_decode( esc_attr($_REQUEST['settings']) ) ), esc_attr($_REQUEST['type']) );
      die();
    }
    ...
    public function shortcode( $settings, $post ) {
      ...
      if ( ! is_object( $post ) ) {
        $post_id = isset( $_GET['post'] ) ? esc_attr($_GET['post']) : ( isset( $_GET['post_ID'] ) ? esc_attr($_GET['post_ID']) : 0 );
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        ...
        return apply_filters( 'ot_upload_text', esc_html__( 'Send to OptionTree', 'baltazar' ) );
