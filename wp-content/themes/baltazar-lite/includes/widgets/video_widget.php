 ...
 */
add_action( 'widgets_init', 'baltazar_video_widget' );
/* Function that registers our widget. */
function baltazar_video_widget() {
	register_widget( 'baltazar_videos' );
}
class baltazar_videos extends WP_Widget {
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'videos', 'description' => esc_attr__('Displays the post image and title.','baltazar'));
		/* Create the widget. */
		parent::__construct( 'videos-widget',esc_attr__('Baltazar - Premium Video','baltazar'), $widget_ops, '' );
	...
	function widget( $args, $instance ) {
		...
		baltazar_security($before_widget); 
		
		if ( $title ) baltazar_security($before_title . $title . $after_title);  ?>
		<style scoped>
		...
		<div class="widgett <?php if(!empty($instance['self_video'])) echo 'self'; ?> id-<?php echo esc_attr($this->id); ?>">	
			<div class="video">
				...
				<?php if(!empty($instance['display_text'])) { ?>
					<div class="wttitle"><a target="_blank" href="<?php echo esc_attr($instance['link']); ?>" rel="bookmark" title="<?php esc_attr_e('Permanent Link to','baltazar')?> <?php echo esc_attr($instance['text']); ?>"><?php echo esc_attr($instance['text']);?></a></div>
				<?php } ?>
		</div>	
		...
		<?php
		echo baltazar_security($after_widget);
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Baltazar Video', 'video' => 'https://www.youtube.com/watch?v=35YZkJaDhh8', 'text' => 'Baltazar Video Text', 'text-top' => '200px','self_video'=>'','auto_play' => '','display_text' => '','link' =>'','self_video' => '','image' => '');
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_attr_e('Title:','baltazar') ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
		</p>
		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'video' )); ?>"><?php esc_attr_e('Video URL:','baltazar') ?></label>
			...
			<br /><small><?php esc_attr_e('(Youtube or Vimeo or self hosted)','baltazar') ?></small>
		</p>
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'self_video' )); ?>"><?php esc_attr_e('Self hosted video (formats : "mp4", "m4v", "webm", "ogv", "wmv", "flv")?','baltazar') ?></label>
		</p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_name( 'image' )); ?>"><?php esc_attr_e('Image cover for self hosted video:','baltazar') ?></label>
            ...
            <input class="upload_image_button" type="button" value="<?php esc_attr_e('Upload Image','baltazar') ?>" />
        </p>		
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'auto_play' )); ?>"><?php esc_attr_e('Auto play (only for self hosted video)?','baltazar') ?></label>
		</p>		
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'display_text' )); ?>"><?php esc_attr_e('Display Text with link?','baltazar') ?></label>
		</p>		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'link' )); ?>"><?php esc_attr_e('Link URL:','baltazar') ?></label>
			...
			<br /><small><?php esc_attr_e('(Link for the text)','baltazar') ?></small>
		</p>			
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_attr_e('Text:','baltazar') ?></label>
			...
			<br /><small><?php esc_attr_e('(Text over the video)','baltazar') ?></small>
		</p>		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text-top' )); ?>"><?php esc_attr_e('Text bottom margin:','baltazar') ?></label>
			...
			<br /><small><?php esc_attr_e('(Bottom margin for text with px (200px))','baltazar') ?></small>
		</p>		
		<?php
	}
